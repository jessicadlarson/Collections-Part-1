...
public class Exercises {
	...
	public List<String> array2List(String[] stringArray) {
		// create an empty list of Strings to return
		List<String> resultList = new ArrayList<String>();
		// iterate through array using for-each (no index)
	...
	public String[] list2Array(List<String> stringList) {
		String[] resultString = new String[(stringList.size())];
		for(int i = 0; i < stringList.size(); i++) {
			resultString[i] = stringList.get(i);
	...
	public List<String> no4LetterWords(String[] stringArray) {
		// create an empty list of Strings to return
			...
			// iterate through array using for-each (no index)
			for (String currentString : stringArray) {
				//add this string to the list if the length doesn't equal four
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		for(int currentInt : intArray) {
			double replace = currentInt;
			double divide = replace / 2;
	...
	public List<Integer> oddOnly(Integer[] integerArray) {
		...
		for(Integer currentInt : integerArray) {
			if(currentInt % 2 != 0) {
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		...
		for (Integer currentInt : integerArray) {
			if(currentInt % 3 == 0 && currentInt % 5 == 0) {
				...
				resultList.add(currentInt.toString());
			}
		}
		return resultList;
	}
